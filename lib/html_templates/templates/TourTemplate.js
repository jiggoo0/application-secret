...
/**
 * @title generateTourContent
 * @description สร้าง HTML Content สำหรับการจองทัวร์ (ไม่รวม DOCTYPE และ Base Layout)
 * @param {object} data - ข้อมูลการจองที่ผ่านการ Map และจัดรูปแบบแล้ว (BookingSchema)
...
export function generateTourContent(data = {}) {
  ...
  const accent = PDF_COLORS.ACCENT; // Teal
  const success = PDF_COLORS.SUCCESS;
  const highlight_bg = PDF_COLORS.HIGHLIGHT_BG;
  const { BORDER, MUTED } = PDF_COLORS;

  // 💡 Template CSS (Tour Specific)
  const componentStyle = `
    <style>
        ...
        .rate-box .label { font-size: 11pt; color: ${MUTED}; }
 ...
 */
export default generateTourContent;
